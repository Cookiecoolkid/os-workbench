...
void exec_command(char *command, char *arg1, char *arg2, char *arg3, char *arg4,
    ...
    if (pid < 0) {
        ...
        execlp(command, command, arg1, arg2, arg3, arg4, arg5, arg6, arg7,
               NULL);
        perror("execlp");
        exit(EXIT_FAILURE);
    } else {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (get_function) {
            ...
            snprintf(expr_line, sizeof(expr_line), "int %s() { return %s;}",
                     name, line);
...
#ifdef __x86_64__
        exec_command("gcc", "-shared", "-o", lib_path, src_path, "-L/tmp",
                     dl_option, NULL);
#else
        exec_command("gcc", "-m32", "-shared", "-o", lib_path, src_path,
                     "-L/tmp", dl_option);
#endif
        if (get_function) {
            // load function.so
            snprintf(func_lib_path[func_index],
                     sizeof(func_lib_path[func_index]), "%s", lib_path);
            func_index++;
            ...
            for (int i = 0; i < func_index; i++) {
                            void *handle = dlopen(func_lib_path[i], RTLD_NOW |
            RTLD_GLOBAL); if (handle == NULL) { fprintf(stderr, "dlopen: %s\n",
