...
MatMulArgs matmul_args[NUM_THREADS];
spinlock_t lk;
int idx = 0;
...
void matmul_forward(float* out,
		...
		for (int b = 0; b < B; b++) {
				for (int t = 0; t < T; t++) {
						...
						matmul_args[idx].out_bt = out_bt;
						matmul_args[idx].inp_bt = inp_bt;
						matmul_args[idx].weight = weight;
						matmul_args[idx].bias = bias;
						matmul_args[idx].C = C;
						matmul_args[idx++].OC = OC;
...
void matmul_forward(float* out,
    ...
    // out will be (B,T,OC)
		// printf("RIGHT = {.out=%p, .inp=%p, .weight=%p, .bias=%p, .B=%d, .T=%d, .C=%d, .OC=%d}\n",
