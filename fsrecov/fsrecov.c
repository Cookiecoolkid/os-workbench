...
void *clus2sec(u32 n) {
    u32 RootDirSectors =
        ((hdr->BPB_RootEntCnt * 32) + (hdr->BPB_BytsPerSec - 1)) /
        hdr->BPB_BytsPerSec;
    u32 FirstDataSec = hdr->BPB_RsvdSecCnt +
                       (hdr->BPB_NumFATs * hdr->BPB_FATSz32) + RootDirSectors;
...
int count_cluster_bmp(u32 clusId) {
    ...
    for (int i = 0; i < cluster_size - 2; i++)
        if (data[i] == 'B' && data[i + 1] == 'M' && data[i + 2] == 'P')
            count++;
...
int main(int argc, char *argv[]) {
    ...
    cluster_num = (hdr->BPB_TotSec32 - hdr->BPB_RsvdSecCnt -
